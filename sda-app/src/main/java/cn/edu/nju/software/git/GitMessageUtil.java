...
import cn.edu.nju.software.sda.app.entity.common.github.CommitDetail;
import cn.edu.nju.software.sda.app.entity.common.github.CommitFile;
import cn.edu.nju.software.sda.app.entity.common.github.CommitStats;
import cn.edu.nju.software.sda.app.utils.HttpClientUtil;
...
//        System.out.println(getGitCommitInfo(1,"");

        Repository existingRepo = new FileRepositoryBuilder()
                .setGitDir(new File("E:\\workspace\\other-project\\JSH_ERP\\.git"))
                .build();
        File file = new File("gitmsg.txt");
        FileUtils.write(file, "");
        try (Git git = new Git(existingRepo)) {
            Iterable<RevCommit> commits = git.log().all().call();
            int count = 0;
            for (RevCommit commit : commits) {
                FileUtils.writeStringToFile(file, count + "  ", true);

                FileUtils.writeStringToFile(file, DateFormatUtils.format(((long)commit.getCommitTime())*1000, "yyyy-MM-dd hh:mm:ss") + "  ", true);
                FileUtils.writeStringToFile(file, commit.getFullMessage(), true);
                count++;
